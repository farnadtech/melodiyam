<x-layouts.app :title="$track->title">
    <div class="p-4 lg:p-8 space-y-8">
        ...
        @php
            $waveformTrackPayload = [
                'id' => $track->id,
                'title' => $track->title,
                'artist' => $track->artist->display_name ?? '',
                'url' => $track->getStreamUrl(),
                'cover' => $track->getCoverUrl(),
                'cover_page' => route('track.show', $track),
                'artist_url' => $track->artist ? route('artist.show', $track->artist) : '',
                'duration' => $track->duration,
                'previewSeconds' => $previewSec ?? 0,
        ...
        <section
            ...
            data-can-play="false"
            data-track-payload='@json($waveformTrackPayload)'
            class="relative"
        >
        ...
        {{-- Comments --}}
        <section
            x-data="trackComments()"
            data-track-id="{{ $track->id }}"
            data-comment-count="{{ $comments->count() }}"
            data-comment-url="{{ route('comment.store') }}"
        >
