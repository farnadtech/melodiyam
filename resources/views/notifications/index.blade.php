        ...
        @else
            <div class="space-y-2">
                @foreach($notifications as $notification)
                    <div class="glass-card rounded-2xl p-4 flex items-start gap-4 {{ is_null($notification->read_at) ? 'border-r-2 border-primary-500' : '' }}">
                        {{-- Icon --}}
                        <div class="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center
                            {{ is_null($notification->read_at) ? 'bg-primary-100 dark:bg-primary-900/50 text-primary-600 dark:text-primary-400' : 'bg-surface-100 dark:bg-surface-800 text-surface-400' }}">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            @if(!empty($notification->data['message']))
                                <p class="text-sm text-surface-500 mt-0.5">{{ $notification->data['message'] }}</p>
                            @endif
