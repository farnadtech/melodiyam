        ...
        >
            ...
            <button 
                @click="if (navigator.share) { 
                    navigator.share({ title: '{{ e($artist->display_name) }}', url: window.location.href }); 
                } else { 
                    navigator.clipboard.writeText(window.location.href); 
                    window.dispatchEvent(new CustomEvent('toast', { detail: { message: 'لینک پروفایل هنرمند کپی شد', type: 'success' } }));
                }"
                class="p-3 rounded-full border border-surface-300 dark:border-surface-600 hover:border-primary-500 transition-colors"
            >
