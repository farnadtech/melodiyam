<x-layouts.app title="ایجاد پادکست جدید">
<div class="p-4 lg:p-8 max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-display font-bold text-surface-900 dark:text-white">ایجاد پادکست جدید</h1>
    </div>
    ...
    <form action="{{ route('artist.podcasts.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="glass-card rounded-2xl p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category" class="w-full rounded-xl border border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                        ...
                        @foreach($genres as $genre)
                            <option value="{{ $genre->name_fa }}" @selected(old('category') === $genre->name_fa)>
                                {{ $genre->name_fa }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1.5">زبان</label>
                    <select name="language" class="w-full rounded-xl border border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-800 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                        <option value="fa" @selected(old('language') === 'fa')>فارسی</option>
                        <option value="en" @selected(old('language') === 'en')>انگلیسی</option>
                    </select>
                </div>
            </div>
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="btn-primary px-8 py-3">ایجاد پادکست</button>
