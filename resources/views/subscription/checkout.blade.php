...
<div class="p-4 lg:p-8 max-w-xl mx-auto space-y-6" x-data="{
    ...
    applyCoupon() {
        ...
        fetch('{{ route('coupon.validate') }}', {
            ...
            body: JSON.stringify({ code: this.couponCode, amount: this.originalPrice, category: 'plans' })
        })
        .then(r => r.json())
        .then(data => {
            this.loading = false;
    ...
    @if(session('error'))
    <div class="p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-sm">{{ session('error') }}</div>
    @endif
    ...
    <div class="glass-card rounded-2xl p-6 space-y-5">
        ...
        <div class="space-y-2">
            ...
            {{-- Each gateway as a separate radio --}}
        ...
        {{-- Wallet form --}}
        <form x-show="payMethod === 'wallet'" action="{{ route('subscription.pay') }}" method="POST">
            @csrf
        ...
        @foreach($activeGateways as $gkey => $glabel)
        <form x-show="payMethod === 'gateway_{{ $gkey }}'" action="{{ route('subscription.pay') }}" method="POST">
            @csrf
            <input type="hidden" name="plan_id" value="{{ $plan->id }}">
            <input type="hidden" name="coupon_code" :value="couponCode">
        ...
        <div class="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 text-xs text-center">
            درگاه پرداخت آنلاین فعال نیست. فقط پرداخت از کیف پول امکان‌پذیر است.
        </div>
        @endif

    </div>

    <a href="{{ route('premium') }}" class="text-sm text-surface-500 hover:text-primary-500 text-center block">← بازگشت</a>
</div>
</x-layouts.app>
